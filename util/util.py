...
def save_ema_checkpoint(state, is_best, sav_path, filename='ema_model_last.pth.tar'):
    filename = join(sav_path, filename)
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, join(sav_path, 'ema_model_best.pth.tar'))
...
def extract_text_lseg_feature(labelset, args):
    '''extract CLIP text features.'''
...
def extract_text_openseg_feature(labelset, args):
    '''extract CLIP text features.'''
...
def extract_ensemble0_text_feature(labelset, args):
    '''extract CLIP text features.'''

    # a bit of prompt engineering
    if hasattr(args, 'prompt_eng') and args.prompt_eng:
        print('Use house prompt engineering: a XX in a scene')
        labelset = [ "a " + label + " in a scenario" for label in labelset]
        # labelset = [label for label in labelset]
        # labelset = [ "a " + label for label in labelset]
        if 'scannet_3d' in args.data_root:
            labelset[-1] = 'other'
        if 'matterport_3d' in args.data_root:
            labelset[-2] = 'other'
...
def extract_ensemble1_text_feature(labelset, args):
    '''extract CLIP text features.'''

    # a bit of prompt engineering
    if hasattr(args, 'prompt_eng') and args.prompt_eng:
        print('Use prompt engineering: a XX in a scene')
        labelset = [ "a point cloud of  " + label + " in a scene" for label in labelset]
        if 'scannet_3d' in args.data_root:
            labelset[-1] = 'other'
        if 'matterport_3d' in args.data_root:
            labelset[-2] = 'other'
...
def extract_ensemble2_text_feature(labelset, args):
    '''extract CLIP text features.'''

    # a bit of prompt engineering
    if hasattr(args, 'prompt_eng') and args.prompt_eng:
        print('Use prompt engineering: a XX in a scene')
        labelset = [ "an indoor  " + label + " in a scene" for label in labelset]
        if 'scannet_3d' in args.data_root:
            labelset[-1] = 'other'
        if 'matterport_3d' in args.data_root:
            labelset[-2] = 'other'
...
def extract_ensemble3_text_feature(labelset, args):
    '''extract CLIP text features.'''

    # a bit of prompt engineering
    if hasattr(args, 'prompt_eng') and args.prompt_eng:
        print('Use prompt engineering: a XX in a scene')
        labelset = [ "an indoor  " + label + " in a point cloud" for label in labelset]
        if 'scannet_3d' in args.data_root:
            labelset[-1] = 'other'
        if 'matterport_3d' in args.data_root:
            labelset[-2] = 'other'
