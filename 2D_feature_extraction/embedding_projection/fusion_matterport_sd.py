...
def process_one_scene(data_path, out_dir, args):
    ...
    for img_id, img_dir in enumerate(tqdm(img_dirs)):
        ...
        feat_2d_0 = torch.nn.functional.interpolate(feat_2d[0], size=(512, 640), mode='bicubic', align_corners=False).squeeze(0) # 1280, 240, 320
        feat_2d_1 = torch.nn.functional.interpolate(feat_2d[1], size=(512, 640), mode='bicubic', align_corners=False).squeeze(0) # 1280, 240, 320
        feat_2d_2 = torch.nn.functional.interpolate(feat_2d[2], size=(512, 640), mode='bicubic', align_corners=False).squeeze(0) #  640, 240, 320
...
def main(args):   
    ...
    args.feat_dim = 3200 # stable diffusion feature dimension
    ...
    # data_paths = [i for i in data_paths if 'x8F5xyUWy9e' in i or 'zsNo4HB9uLZ' in i]
    data_paths = [i for i in data_paths if 'dhjEzFoUFzH' in i]
