...
from models.disnet_with_uncertainty_blocks_dinov2_sd import DisNet as Model
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.distributed:
        ...
        if main_process():
            logger.info("multi gpu distributed training ...")
...
def distill(train_loader, model, optimizer, epoch):
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        (output_3d, 
         uncertainty_pred_lseg, 
         output_3d_dinov2, 
         uncertainty_pred_dinov2, 
         output_3d_sd, 
         uncertainty_pred_sd) = model(sinput)
        ...
        if hasattr(args, 'loss_type') and args.loss_type == 'cosine':
            ...
            loss_lseg = torch.nn.MSELoss(reduction="none")(
                output_3d.float(), feat_3d.float())
            # uncertainty_loss_lseg = torch.nn.L1Loss(reduction='none')(
            #     uncertainty_pred_lseg[mask], 
            #     loss_lseg[..., None].detach(),
            # )
        ...
        if hasattr(args, 'loss_dinov2_type') and args.loss_type == 'cosine':
            ...
            loss_dinov2 = torch.nn.MSELoss(reduction='none')(
                output_3d_dinov2.float(), dinov2_feat_3d.float())
            # uncertainty_loss_dinov2 = torch.nn.L1Loss(reduction='none')(
            #     uncertainty_pred_dinov2[mask], 
            #     loss_dinov2[..., None].detach(),
            # )
        ...
        uncertainty_loss_lseg = uncertainty_pred_lseg[mask]
        uncertainty_loss_dinov2 = uncertainty_pred_dinov2[mask]
        uncertainty_loss_sd = uncertainty_pred_sd[mask]
        
        loss_lseg = 0.5 / (uncertainty_loss_lseg ** 2) * loss_lseg + torch.log(1.0 + uncertainty_loss_lseg ** 2)
        loss_dinov2 = 0.5 / (uncertainty_loss_dinov2 ** 2) * loss_dinov2 + torch.log(1.0 + uncertainty_loss_dinov2 ** 2)
        loss_sd = 0.5 / (uncertainty_loss_sd ** 2) * loss_sd + torch.log(1.0 + uncertainty_loss_sd ** 2)
        
        # loss_lseg = 0.5 / (uncertainty_loss_lseg ** 2) * loss_lseg + torch.log(1.0 + uncertainty_loss_lseg)
        # loss_dinov2 = 0.5 / (uncertainty_loss_dinov2 ** 2) * loss_dinov2 + torch.log(1.0 + uncertainty_loss_dinov2)
        # loss_sd = 0.5 / (uncertainty_loss_sd ** 2) * loss_sd + torch.log(1.0 + uncertainty_loss_sd)
        
        # loss_lseg = 0.5 / (uncertainty_loss_lseg ** 2) * loss_lseg + torch.log(uncertainty_loss_lseg + 1e-6)
        # loss_dinov2 = 0.5 / (uncertainty_loss_dinov2 ** 2) * loss_dinov2 + torch.log(uncertainty_loss_dinov2 + 1e-6)
        # loss_sd = 0.5 / (uncertainty_loss_sd ** 2) * loss_sd + torch.log(uncertainty_loss_sd + 1e-6)
        
        loss_distill = loss_lseg.mean() + loss_dinov2.mean() + loss_sd.mean()
        # loss_uncertainty = uncertainty_loss_lseg.mean() + uncertainty_loss_dinov2.mean() + uncertainty_loss_sd.mean()
        # loss = loss_distill + loss_uncertainty
        loss = loss_distill
        ...
        distill_lseg_loss_meter.update(loss_lseg.mean().item(), args.batch_size)
        distill_dinov2_loss_meter.update(loss_dinov2.mean().item(), args.batch_size)
        distill_sd_loss_meter.update(loss_sd.mean().item(), args.batch_size)
