...
from models.disnet_uncertainty_blocks_embedding_url import DisNet as Model
...
def distill(train_loader, model, optimizer, epoch):
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        output_3d, uncertainty_pred = model(sinput)
        ...
        if hasattr(args, 'loss_type') and args.loss_type == 'cosine':
            ...
            cosine_loss = (1 - torch.nn.CosineSimilarity()
                    (output_3d, feat_3d))
        ...
        distill_loss_meter.update(cosine_loss.item(), args.batch_size)
        uncertainty_loss_meter.update(uncertainty_loss.item(), args.batch_size)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'DistillLoss {distill_loss_meter.val:.4f} '
                        'UncertaintyLoss {uncertainty_loss_meter.val:.4f} '.format(
                            epoch + 1, 
                            args.epochs, 
                            i + 1, 
                            len(train_loader),
                            ...
                            distill_loss_meter=distill_loss_meter,
                            uncertainty_loss_meter=uncertainty_loss_meter))
        if main_process():
            writer.add_scalar('loss_train_batch', loss_meter.val, current_iter)
            writer.add_scalar('ditill_loss_train_batch', distill_loss_meter.val, current_iter)
            writer.add_scalar('uncertainty_loss_train_batch', uncertainty_loss_meter.val, current_iter)
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        for batch_data in tqdm(val_loader):
            ...
            output, uncertainty_pred = model(sinput)
