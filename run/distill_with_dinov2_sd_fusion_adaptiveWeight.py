    ...
    for i, batch_data in enumerate(train_loader):
        ...
        if hasattr(args, 'loss_dinov2_type') and args.loss_type == 'cosine':
            loss_dinov2 = (1 - torch.nn.CosineSimilarity()
                    (output_3d_dinov2, dinov2_feat_3d))
            ...
            loss_dinov2 = torch.nn.L1Loss(reduction='none')(output_3d_dinov2, dinov2_feat_3d)
            # uncertainty_loss_dinov2 = torch.nn.L1Loss(reduction='none')(
            #     uncertainty_pred_dinov2[mask], 
            #     loss_dinov2[..., None].detach(),
            # )
        elif hasattr(args, 'loss_dinov2_type') and args.loss_type == 'l2':
            loss_dinov2 = torch.nn.MSELoss(reduction='none')(output_3d_dinov2.float(), dinov2_feat_3d.float())
            # uncertainty_loss_dinov2 = torch.nn.L1Loss(reduction='none')(
            #     uncertainty_pred_dinov2[mask], 
            #     loss_dinov2[..., None].detach(),
            # )
        ...
        if hasattr(args, 'loss_sd_type') and args.loss_type == 'cosine':
            ...
            loss_sd = torch.nn.L1Loss(reduction='none')(F.normalize(output_3d_sd, 2, dim=1), 
                                        F.normalize(sd_feat_3d, 2, dim=1))
            # uncertainty_loss_sd = torch.nn.L1Loss(reduction='none')(
            #     uncertainty_pred_sd[mask], 
            #     loss_sd[..., None].detach(),
            ...
            loss_sd = torch.nn.MSELoss(reduction='none')(F.normalize(output_3d_sd, 2, dim=1), 
                                         F.normalize(sd_feat_3d.float(), 2, dim=1))
            # uncertainty_loss_sd = torch.nn.L1Loss(reduction='none')(
            #     uncertainty_pred_sd[mask], 
            #     loss_sd[..., None].detach(),
            # )
