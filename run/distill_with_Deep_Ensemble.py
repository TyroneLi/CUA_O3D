...
from models.disnet_deep_ensemble import DisNet as Model
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #         val_loader, model, criterion, args.numEnsembles)
        
        loss_train = distill(train_loader, model, optimizer, epoch, args.numEnsembles)
        epoch_log = epoch + 1
        if main_process():
...
def distill(train_loader, model, optimizer, epoch, numEnsembles):
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        all_losses = []
        for idx_ensemble in range(numEnsembles):
        
            i_output_3d = output_3d[idx_ensemble][mask]

            if hasattr(args, 'loss_type') and args.loss_type == 'cosine':
                loss = (1 - torch.nn.CosineSimilarity()
                        (i_output_3d, feat_3d)).mean()
            elif hasattr(args, 'loss_type') and args.loss_type == 'l1':
                loss = torch.nn.L1Loss()(i_output_3d, feat_3d)
            elif hasattr(args, 'loss_type') and args.loss_type == 'l2':
                loss = torch.nn.MSELoss()(i_output_3d.float(), feat_3d.float())
            else:
                raise NotImplementedError
            
            all_losses.append(loss)
        
        total_losses = sum(all_losses) / numEnsembles
        ...
        loss1_meter.update(all_losses[0].item(), args.batch_size)
        loss2_meter.update(all_losses[1].item(), args.batch_size)
        loss3_meter.update(all_losses[2].item(), args.batch_size)
        # loss4_meter.update(all_losses[3].item(), args.batch_size)
        # loss5_meter.update(all_losses[4].item(), args.batch_size)
...
def validate(val_loader, model, criterion, numEnsembles):
    '''Validation.'''
