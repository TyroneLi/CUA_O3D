...
def compute_flops(model, input_sparse_tensor):
    total_flops = 0

    def hook(module, input, output):
        if isinstance(module, ME.MinkowskiConvolution):
            num_active_voxels = input[0].C.size(0)
            kernel_volume = torch.prod(torch.tensor(module.kernel_generator.kernel_size))
            # layer_flops = (
            #     num_active_voxels
            #     * kernel_volume
            #     * module.in_channels
    ...
    # Register hooks for all MinkowskiConvolution layers
    hooks = []
...
def distill(train_loader, model, optimizer, epoch):
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        flops, macs = profile(model, (sinput, ))
        # batchsize=1 -> flops=330013568.0, macs=10688.0
        import pdb;pdb.set_trace()
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        for batch_data in tqdm(val_loader):
            ...
            flops = compute_flops(model, sinput)
            print(f"Total FLOPs: {flops}")
            
            # # stat(model, (1, 148880, 3))
            # # input_shape = (1, 1, 148880, 3)
            # input_shape = (148880, 3)
            ...
            # print(f"Before forward pass: {torch.cuda.memory_allocated() / 1024 / 1024} MB")
            
            # output, _, _ = model(sinput)
            
            # print(f"After forward pass: {torch.cuda.memory_allocated() / 1024 / 1024} MB")
