...
from util.util import export_pointcloud, get_palette, \
    convert_labels_with_palette, extract_text_feature, visualize_labels, \
        extract_ensemble1_text_feature, extract_ensemble2_text_feature, \
            extract_ensemble3_text_feature, extract_ensemble0_text_feature
from tqdm import tqdm
from run.distill_with_amradio import get_model
...
def precompute_text_related_properties(labelset_name):
    ...
    # en0_text_features = extract_ensemble0_text_feature(labelset, args)
    # en1_text_features = extract_ensemble1_text_feature(labelset, args)
    # en2_text_features = extract_ensemble2_text_feature(labelset, args)
    # en3_text_features = extract_ensemble3_text_feature(labelset, args)
        ...
        for rep_i in range(args.test_repeats):
            ...
            for i, (coords, feat, label, feat_3d, mask, inds_reverse) in enumerate(tqdm(val_data_loader)):
                ...
                if feature_type == 'distill':
                    predictions, predictions_amradio = model(sinput)
