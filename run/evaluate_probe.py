...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if os.path.isfile(args.model_path):
        ...
        nuscenes_state_dict = checkpoint['state_dict']
        new_nuscenes_state_dict = dict()
        for k, v in nuscenes_state_dict.items():
            if 'module.' in k:
                new_nuscenes_state_dict[k.split(".", 1)[1]] = v
            else:
                new_nuscenes_state_dict[k] = v
        
        # model.load_state_dict(checkpoint['state_dict'], strict=True)
        model.load_state_dict(new_nuscenes_state_dict, strict=True)
    ...
    with torch.no_grad():
        ...
        for rep_i in range(args.test_repeats):
            ...
            for i, (coords, feat, label, inds_reverse) in enumerate(tqdm(val_loader)):
                ...
                # predictions = (predictions[0] + predictions[1] + predictions[2]) / 3.0
