...
from util.util import export_pointcloud, get_palette, \
    convert_labels_with_palette, extract_text_feature, visualize_labels, \
        extract_text_lseg_feature, extract_text_openseg_feature
from tqdm import tqdm
from run.distill_with_lseg_openseg import get_model
...
def obtain_text_features_lseg_openseg(labelset_name):
    ...
    text_lseg_features = extract_text_lseg_feature(labelset, args)
    text_openeg_features = extract_text_openseg_feature(labelset, args)
    # labelset.append('unknown')
    labelset.append('unlabeled')
    
    return text_lseg_features, text_openeg_features, labelset, mapper, palette
...
def evaluate(model, val_data_loader, labelset_name='scannet_3d'):
    ...
    lseg_text_features, openseg_text_features, labelset, mapper, palette = \
        obtain_text_features_lseg_openseg(labelset_name)
        ...
        for rep_i in range(args.test_repeats):
            ...
            for i, (coords, feat, label, feat_3d, mask, inds_reverse) in enumerate(tqdm(val_data_loader)):
                ...
                if feature_type == 'distill':
                    # _, predictions = model(sinput)
                    lseg_predictions, openseg_predictions = model(sinput)
                    # predictions = (lseg_predictions + openseg_predictions) / 2.0
                    
                    lseg_predictions = lseg_predictions[inds_reverse, :]
                    lseg_predictions = lseg_predictions.half() @ lseg_text_features.t()
                    
                    openseg_predictions = openseg_predictions[inds_reverse, :]
                    openseg_predictions = openseg_predictions.half() @ openseg_text_features.t()
