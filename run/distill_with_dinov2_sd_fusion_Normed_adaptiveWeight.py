...
from models.disnet_with_dinov2_sd_fusion import DisNet as Model
from tqdm import tqdm
from models.adaptiveWeightLoss import AutomaticWeightedLoss

best_iou = 0.0

def feat_normalize(in_feat):
    assert len(in_feat.shape) == 2
    in_feat = in_feat / in_feat.norm(dim=-1, keepdim=True)
    return in_feat
...
def get_parser():
    ...
    awl_model_dir = os.path.join(cfg.save_path, 'awl_model')
    result_dir = os.path.join(cfg.save_path, 'result')
    os.makedirs(model_dir, exist_ok=True)
    os.makedirs(awl_model_dir, exist_ok=True)
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    awl_model = AutomaticWeightedLoss(num=3)
    ...
    if main_process():
        ...
        logger.info(model)
        logger.info(awl_model)

    # ####################### Optimizer ####################### #
    # optimizer = torch.optim.Adam(model.parameters(), lr=args.base_lr)
    optimizer = torch.optim.Adam([
                {'params': model.parameters()},
                {'params': awl_model.parameters(), 'weight_decay': 0}
            ], lr=args.base_lr)
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            assert os.path.exists(args.resume.replace('/model/', '/awl_model/'))
            awl_checkpoint = torch.load(
                args.resume.replace('/model/', '/awl_model/'), 
                map_location=lambda storage, loc: storage.cuda())
            # args.start_epoch = checkpoint['epoch']
            awl_model.load_state_dict(awl_checkpoint['state_dict'], strict=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_train = distill(train_loader, model, optimizer, epoch, awl_model)
        epoch_log = epoch + 1
        if main_process():
        ...
        if (epoch_log % args.save_freq == 0) and main_process():
            ...
            save_checkpoint(
                {
                    'epoch': epoch_log,
                    'state_dict': awl_model.state_dict(),
                    'optimizer': optimizer.state_dict(),
                    'best_iou': best_iou
                }, is_best, os.path.join(args.save_path, 'awl_model')
            )
    if main_process():
        writer.close()
...
def distill(train_loader, model, optimizer, epoch, awl_model):
    ...
    loss_lseg_meter = AverageMeter()
    loss_dinov2_meter = AverageMeter()
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        (coords, feat, label_3d, feat_3d, dinov2_feat_3d, sd_feat_3d, mask) = batch_data
        ...
        dinov2_feat_3d = dinov2_feat_3d.cuda(non_blocking=True)
        ...
        output_3d, output_3d_dinov2, output_3d_sd = model(sinput)
        ...
        if hasattr(args, 'loss_type') and args.loss_type == 'cosine':
            loss_lseg = (1 - torch.nn.CosineSimilarity()
                    (output_3d, feat_3d))
            # uncertainty_loss_lseg = torch.nn.L1Loss(reduction='none')(
            #     uncertainty_pred_lseg[mask], 
            #     loss_lseg[..., None].detach(),
            # )
        elif hasattr(args, 'loss_type') and args.loss_type == 'l1':
            loss_lseg = torch.nn.L1Loss(reduction='none')(output_3d, feat_3d)
            # uncertainty_loss_lseg = torch.nn.L1Loss(reduction='none')(
            #     uncertainty_pred_lseg[mask], 
            #     loss_lseg[..., None].detach(),
            # )
        elif hasattr(args, 'loss_type') and args.loss_type == 'l2':
            loss_lseg = torch.nn.MSELoss(reduction='none')(output_3d.float(), feat_3d.float())
            # uncertainty_loss_lseg = torch.nn.L1Loss(reduction='none')(
            #     uncertainty_pred_lseg[mask], 
            #     loss_lseg[..., None].detach(),
            # )
        ...
        if hasattr(args, 'loss_dinov2_type') and args.loss_type == 'cosine':
            loss_dinov2 = (1 - torch.nn.CosineSimilarity()
                    (output_3d_dinov2, 
                     feat_normalize(dinov2_feat_3d)))
            # uncertainty_loss_dinov2 = torch.nn.L1Loss(reduction='none')(
            #     uncertainty_pred_dinov2[mask], 
            #     loss_dinov2[..., None].detach(),
            # )
        elif hasattr(args, 'loss_dinov2_type') and args.loss_type == 'l1':
            loss_dinov2 = torch.nn.L1Loss(reduction='none')(
                output_3d_dinov2, 
                feat_normalize(dinov2_feat_3d))
            # uncertainty_loss_dinov2 = torch.nn.L1Loss(reduction='none')(
            #     uncertainty_pred_dinov2[mask], 
            #     loss_dinov2[..., None].detach(),
            # )
        elif hasattr(args, 'loss_dinov2_type') and args.loss_type == 'l2':
            loss_dinov2 = torch.nn.MSELoss(reduction='none')(
                output_3d_dinov2.float(), 
                feat_normalize(dinov2_feat_3d).float())
            # uncertainty_loss_dinov2 = torch.nn.L1Loss(reduction='none')(
            #     uncertainty_pred_dinov2[mask], 
            #     loss_dinov2[..., None].detach(),
            # )
        ...
        if hasattr(args, 'loss_sd_type') and args.loss_type == 'cosine':
            loss_sd = (1 - torch.nn.CosineSimilarity()
                    (output_3d_sd, 
                     feat_normalize(sd_feat_3d)))
            # uncertainty_loss_sd = torch.nn.L1Loss(reduction='none')(
            #     uncertainty_pred_sd[mask], 
            #     loss_sd[..., None].detach(),
            # )
        elif hasattr(args, 'loss_sd_type') and args.loss_type == 'l1':
            loss_sd = torch.nn.L1Loss(reduction='none')(
                output_3d_sd, 
                feat_normalize(sd_feat_3d))
            # uncertainty_loss_sd = torch.nn.L1Loss(reduction='none')(
            #     uncertainty_pred_sd[mask], 
            ...
            loss_sd = torch.nn.MSELoss(reduction='none')(
                output_3d_sd, 
                feat_normalize(sd_feat_3d.float()))
            # uncertainty_loss_sd = torch.nn.L1Loss(reduction='none')(
            #     uncertainty_pred_sd[mask], 
            #     loss_sd[..., None].detach(),
            # )
        ...
        # import pdb;pdb.set_trace()
        # loss_lseg = loss_lseg * torch.exp(uncertainty_loss_lseg * (-1.0)) * uncertainty_loss_dinov2 * uncertainty_loss_sd
        # loss_dinov2 = loss_dinov2 * torch.exp(uncertainty_loss_dinov2 * (-1.0)) * uncertainty_loss_lseg * uncertainty_loss_sd
        # loss_sd = loss_sd * torch.exp(uncertainty_loss_sd * (-1.0)) * uncertainty_loss_lseg * uncertainty_loss_dinov2
        
        # loss_lseg = loss_lseg * torch.exp(uncertainty_loss_lseg * (-1.0))
        # loss_dinov2 = loss_dinov2 * torch.exp(uncertainty_loss_dinov2 * (-1.0))
        # loss_sd = loss_sd * torch.exp(uncertainty_loss_sd * (-1.0))
        
        loss_lseg = loss_lseg.mean()
        loss_dinov2 = loss_dinov2.mean()
        loss_sd = loss_sd.mean()
        
        # uncertainty_loss_lseg = uncertainty_loss_lseg.mean()
        # uncertainty_loss_dinov2 = uncertainty_loss_dinov2.mean()
        # uncertainty_loss_sd = uncertainty_loss_sd.mean()
        
        # loss = loss_lseg + loss_dinov2 + loss_sd + uncertainty_loss_lseg + uncertainty_loss_dinov2 + uncertainty_loss_sd
        # loss = loss_lseg + loss_dinov2 + loss_sd
        
        loss = awl_model([loss_lseg, loss_dinov2, loss_sd])
        ...
        loss_lseg_meter.update(loss_lseg.item(), args.batch_size)
        loss_dinov2_meter.update(loss_dinov2.item(), args.batch_size)
        loss_sd_meter.update(loss_sd.item(), args.batch_size)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'LossLseg {loss_lseg_meter.val:.4f} '
                        'LossDinoV2 {loss_dinov2_meter.val:.4f} '
                        'LossSD {loss_sd_meter.val:.4f} '
                        'AML_Param_1 {aml_model_params_1:.4f} '
                        'AML_Param_2 {aml_model_params_2:.4f} '
                        'AML_Param_3 {aml_model_params_3:.4f} '.format(epoch + 1, args.epochs, i + 1, len(train_loader),
                                                            batch_time=batch_time, data_time=data_time,
                                                            remain_time=remain_time,
                                                            loss_meter=loss_meter,
                                                            loss_lseg_meter=loss_lseg_meter,
                                                            loss_dinov2_meter=loss_dinov2_meter,
                                                            loss_sd_meter=loss_sd_meter,
                                                            aml_model_params_1=awl_model.params[0],
                                                            aml_model_params_2=awl_model.params[1],
                                                            aml_model_params_3=awl_model.params[2]))
        if main_process():
