...
from dataset.feature_loader_with_lseg_openseg import FusedFeatureLoader, collation_fn
from dataset.point_loader import Point3DLoader, collation_fn_eval_all
from models.disnet_with_lseg_openseg import DisNet as Model
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    train_data = FusedFeatureLoader(datapath_prefix=args.data_root,
                                    ...
                                    datapath_prefix_feat_extra=args.data_root_2d_fused_feature_openseg,
...
def obtain_text_features_lseg_openseg():
    '''obtain the CLIP text feature and palette.'''
    ...
    clip_file_name_lseg = 'saved_text_embeddings/clip_{}_labels{}.pt'.format(dataset_name, '_512')
    logger.info('Load pre-computed embeddings from {}'.format(clip_file_name_lseg))
    text_features_lseg = torch.load(clip_file_name_lseg).cuda()
    clip_file_name_openseg = 'saved_text_embeddings/clip_{}_labels{}.pt'.format(dataset_name, '_768')
    logger.info('Load pre-computed embeddings from {}'.format(clip_file_name_openseg))
    text_features_openseg = torch.load(clip_file_name_openseg).cuda()

    return text_features_lseg, text_features_openseg
...
def distill(train_loader, model, optimizer, epoch):
    ...
    loss_openseg_meter = AverageMeter()
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        (coords, feat, label_3d, feat_3d, openseg_feat_3d, mask) = batch_data
        ...
        openseg_feat_3d = openseg_feat_3d.cuda(non_blocking=True)
        
        output_3d, output_3d_openseg = model(sinput)
        output_3d = output_3d[mask]
        output_3d_openseg = output_3d_openseg[mask]
        ...
        if hasattr(args, 'loss_type') and args.loss_type == 'cosine':
            ...
            loss_openseg = (1 - torch.nn.CosineSimilarity()
                    (output_3d_openseg, openseg_feat_3d)).mean()
        ...
        loss_openseg_meter.update(loss_openseg.item(), args.batch_size)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'LossOpenSeg {loss_openseg_meter.val:.4f} '.format(epoch + 1, args.epochs, i + 1, len(train_loader),
                                                            batch_time=batch_time, data_time=data_time,
                                                            remain_time=remain_time,
                                                            loss_meter=loss_meter,
                                                            loss_lseg_meter=loss_lseg_meter,
                                                            loss_openseg_meter=loss_openseg_meter))
        if main_process():
...
def validate(val_loader, model, criterion):
    ...
    lseg_txt, openseg_txt = obtain_text_features_lseg_openseg()
    ...
    with torch.no_grad():
        for batch_data in tqdm(val_loader):
            ...
            output_lseg, output_openseg = model(sinput)
            
            output_lseg = output_lseg[inds_reverse, :]
            output_lseg = output_lseg.half() @ lseg_txt.t()
            
            output_openseg = output_openseg[inds_reverse, :]
            output_openseg = output_openseg.half() @ openseg_txt.t()
            
            output = (output_lseg + output_openseg) / 2.0
