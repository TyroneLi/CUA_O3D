...
from models.disnet_with_dinov2_sd_SepProbSeg import DisNet as Model
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        output_lseg, output_dinov2, output_sd = model(sinput)
        loss = criterion(output_lseg, label) + criterion(output_dinov2, label) + criterion(output_sd, label)

        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        output = (output_lseg + output_dinov2 + output_sd) / 3.0
