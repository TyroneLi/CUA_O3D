...
from dataset.feature_loader_with_dinov2_sd_amradio import FusedFeatureLoader, collation_fn
from dataset.point_loader_with_dinov2 import Point3DLoader, collation_fn_eval_all
from models.disnet_with_dinov2_sd_amradio import DisNet as Model
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    awl_model = AutomaticWeightedLoss(num=4)
    ...
    train_data = FusedFeatureLoader(datapath_prefix=args.data_root,
                                    ...
                                    datapath_prefix_feat_amradio=args.data_root_2d_fused_feature_amradio,
...
def distill(train_loader, model, optimizer, epoch, awl_model):
    ...
    loss_amradio_meter = AverageMeter()
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        (coords, feat, label_3d, feat_3d, dinov2_feat_3d, sd_feat_3d, amradio_feat_3d, mask) = batch_data
        ...
        amradio_feat_3d = amradio_feat_3d.cuda(non_blocking=True)
        
        output_3d, output_3d_dinov2, output_3d_sd, output_3d_amradio = model(sinput)
        output_3d = output_3d[mask]
        output_3d_dinov2 = output_3d_dinov2[mask]
        output_3d_sd = output_3d_sd[mask]
        output_3d_amradio = output_3d_amradio[mask]
        ...
        if hasattr(args, 'loss_type') and args.loss_type == 'cosine':
            ...
            loss_lseg = torch.nn.L1Loss()(output_3d, feat_3d)
        elif hasattr(args, 'loss_type') and args.loss_type == 'l2':
            loss_lseg = torch.nn.MSELoss()(output_3d.float(), feat_3d.float())
        else:
            raise NotImplementedError
        
        if hasattr(args, 'loss_dinov2_type') and args.loss_type == 'cosine':
            loss_dinov2 = (1 - torch.nn.CosineSimilarity()
                    (output_3d_dinov2, dinov2_feat_3d)).mean()
        elif hasattr(args, 'loss_dinov2_type') and args.loss_type == 'l1':
            loss_dinov2 = torch.nn.L1Loss()(output_3d_dinov2, dinov2_feat_3d)
        elif hasattr(args, 'loss_dinov2_type') and args.loss_type == 'l2':
            loss_dinov2 = torch.nn.MSELoss()(output_3d_dinov2.float(), dinov2_feat_3d.float())
        else:
            raise NotImplementedError
        ...
        if hasattr(args, 'loss_amradio_type') and args.loss_type == 'cosine':
            loss_amradio = (1 - torch.nn.CosineSimilarity()
                    (output_3d_amradio, amradio_feat_3d)).mean()
        elif hasattr(args, 'loss_amradio_type') and args.loss_type == 'l1':
            loss_amradio = torch.nn.L1Loss()(output_3d_amradio, amradio_feat_3d)
        elif hasattr(args, 'loss_amradio_type') and args.loss_type == 'l2':
            loss_amradio = torch.nn.MSELoss()(output_3d_amradio.float(), amradio_feat_3d.float())
        else:
            raise NotImplementedError

        # loss = loss_lseg + loss_dinov2 + loss_sd + loss_amradio
        loss = awl_model([loss_lseg, loss_dinov2, loss_sd, loss_amradio])
        ...
        loss_amradio_meter.update(loss_amradio.item(), args.batch_size)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'LossAMRADIO {loss_amradio_meter.val:.4f} '
                        'AML_Param_1 {aml_model_params_1:.4f} '
                        'AML_Param_2 {aml_model_params_2:.4f} '
                        'AML_Param_3 {aml_model_params_3:.4f} '
                        'AML_Param_4 {aml_model_params_4:.4f} '.format(epoch + 1, args.epochs, i + 1, len(train_loader),
