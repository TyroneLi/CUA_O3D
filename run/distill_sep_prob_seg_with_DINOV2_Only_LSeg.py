...
from models.disnet_with_dinov2_SepProbSeg import DisNet as Model
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.init:
        if os.path.isfile(args.init):
            if main_process():
                logger.info("=> loading checkpoint '{}'".format(args.init))
            # checkpoint = torch.load(args.resume)
            checkpoint = torch.load(
                args.init, map_location=lambda storage, loc: storage.cuda())
            ...
            if main_process():
                logger.info(
                    "=> no checkpoint found at '{}'".format(args.init))
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        output_lseg, output_dinov2 = model(sinput)
        # loss = criterion(output_lseg, label) + criterion(output_dinov2, label)
        # loss = criterion(output_dinov2, label)
        loss = criterion(output_lseg, label)

        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        # output = (output_lseg + output_dinov2) / 2.0
        # output = output_dinov2
        output = output_lseg
