...
from run.distill_with_MCDropOut import get_model
...
def evaluate(model, val_data_loader, labelset_name='scannet_3d'):
    ...
    with torch.no_grad():
        ...
        for layer in model.net3d.children():
            for module in layer.modules():
                # print(module)
                if isinstance(module, ME.MinkowskiDropout):
                    print(module, dir(module.MODULE))
                    module.MODULE.p = 0.5
                    module.train()
        ...
        for rep_i in range(args.test_repeats):
            ...
            for i, (coords, feat, label, feat_3d, mask, inds_reverse) in enumerate(tqdm(val_data_loader)):
                ...
                if feature_type == 'distill':
                    ...
                    predictions = torch.stack(all_predctions, dim=0)
