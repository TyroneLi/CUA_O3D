...
def distill(train_loader, model, optimizer, epoch):
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        if hasattr(args, 'loss_type') and args.loss_type == 'cosine':
            ...
            loss = torch.nn.MSELoss()(output_3d.float(), feat_3d.float())
        else:
            raise NotImplementedError
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        for batch_data in tqdm(val_loader):
            ...
            # macs = torchprofile.profile_macs(model, sinput)
            # print(f"MACs: {macs}")
            
            # macs, params = ptflops.get_model_complexity_info(model, (sinput,), print_per_layer_stat=True)
