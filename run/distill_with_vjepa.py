...
from dataset.feature_loader_with_vjepa import FusedFeatureLoader, collation_fn
from dataset.point_loader_with_vjepa import Point3DLoader, collation_fn_eval_all
from models.disnet_with_vjepa import DisNet as Model
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    train_data = FusedFeatureLoader(datapath_prefix=args.data_root,
                                    ...
                                    datapath_prefix_feat_vjepa=args.data_root_2d_fused_feature_vjepa,
...
def distill(train_loader, model, optimizer, epoch):
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        (coords, feat, label_3d, feat_3d, vjepa_feat_3d, mask) = batch_data
        ...
        vjepa_feat_3d = vjepa_feat_3d.cuda(non_blocking=True)
        
        output_3d, output_3d_vjepa = model(sinput)
        output_3d = output_3d[mask]
        output_3d_vjepa = output_3d_vjepa[mask]
        ...
        if hasattr(args, 'loss_dinov2_type') and args.loss_type == 'cosine':
            loss_vjepa = (1 - torch.nn.CosineSimilarity()
                    (output_3d_vjepa, vjepa_feat_3d)).mean()
        elif hasattr(args, 'loss_dinov2_type') and args.loss_type == 'l1':
            loss_vjepa = torch.nn.L1Loss()(output_3d_vjepa, vjepa_feat_3d)
        elif hasattr(args, 'loss_dinov2_type') and args.loss_type == 'l2':
            loss_vjepa = torch.nn.MSELoss()(output_3d_vjepa.float(), vjepa_feat_3d.float())
        else:
            raise NotImplementedError

        loss = loss_lseg + loss_vjepa
        ...
        loss_vjepa_meter.update(loss_vjepa.item(), args.batch_size)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'LossVjepa {loss_vjepa_meter.val:.4f} '.format(epoch + 1, args.epochs, i + 1, len(train_loader),
                                                            batch_time=batch_time, data_time=data_time,
                                                            remain_time=remain_time,
                                                            loss_meter=loss_meter,
                                                            loss_lseg_meter=loss_lseg_meter,
                                                            loss_vjepa_meter=loss_vjepa_meter))
        if main_process():
