...
class FusedFeatureLoader(Point3DLoader):
    ...
    def __getitem__(self, index_long):
        ...
        if len(processed_data.keys())==2:
            ...
            dinov2_feat_3d, feat_3d, mask_chunk = dinov2_processed_data['feat'], processed_data['feat'], processed_data['mask_full']
        ...
        if self.eval_all:
            return coords, feats, labels, feat_3d, dinov2_feat_3d, mask, torch.from_numpy(inds_reconstruct).long()
        return coords, feats, labels, feat_3d, dinov2_feat_3d, mask
...
def collation_fn(batch):
    ...
    '''
    coords, feats, labels, feat_3d, dinov2_feat_3d, mask_chunk = list(zip(*batch))

    for i in range(len(coords)):
        coords[i][:, 0] *= i

    return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
        torch.cat(feat_3d), torch.cat(dinov2_feat_3d), torch.cat(mask_chunk)
...
def collation_fn_eval_all(batch):
    ...
    '''
    coords, feats, labels, feat_3d, dinov2_feat_3d, mask, inds_recons = list(zip(*batch))
    ...
    return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
        torch.cat(feat_3d), torch.cat(dinov2_feat_3d), torch.cat(mask), torch.cat(inds_recons)