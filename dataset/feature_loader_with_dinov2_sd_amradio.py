...
class FusedFeatureLoader(Point3DLoader):
    ...
    def __init__(self,
                 ...
                 datapath_prefix_feat_dinov2,
                 datapath_prefix_feat_sd,
                 datapath_prefix_feat_amradio,
                 voxel_size=0.05,
        ...
        self.datapath_feat_dinov2 = datapath_prefix_feat_dinov2
        self.datapath_feat_sd = datapath_prefix_feat_sd
        self.datapath_prefix_feat_amradio = datapath_prefix_feat_amradio
    ...
    def __getitem__(self, index_long):
        ...
        if 'nuscenes' not in self.dataset_name:
            ...
            dinov2_processed_data = torch.load(join(
                self.datapath_feat_dinov2, scene_name+'_%d.pt'%(nn_occur)))

            sd_processed_data = torch.load(join(
                self.datapath_feat_sd, scene_name+'_%d.pt'%(nn_occur)))
            
            amradio_processed_data = torch.load(join(
                self.datapath_prefix_feat_amradio, scene_name+'_%d.pt'%(nn_occur)))
        ...
        if len(processed_data.keys())==2:
            ...
            sd_feat_3d, dinov2_feat_3d, feat_3d, mask_chunk = sd_processed_data['feat'], dinov2_processed_data['feat'], processed_data['feat'], processed_data['mask_full']
            amradio_feat_3d = amradio_processed_data['feat']
            ...
            if self.split != 'train': # val or test set
                ...
                dinov2_feat_3d_new = torch.zeros((locs_in.shape[0], dinov2_feat_3d.shape[1]), dtype=dinov2_feat_3d.dtype)
                dinov2_feat_3d_new[mask] = dinov2_feat_3d
                dinov2_feat_3d = dinov2_feat_3d_new
                
                sd_feat_3d_new = torch.zeros((locs_in.shape[0], sd_feat_3d.shape[1]), dtype=sd_feat_3d.dtype)
                sd_feat_3d_new[mask] = sd_feat_3d
                sd_feat_3d = sd_feat_3d_new
                
                amradio_feat_3d_new = torch.zeros((locs_in.shape[0], amradio_feat_3d.shape[1]), dtype=amradio_feat_3d.dtype)
                amradio_feat_3d_new[mask] = amradio_feat_3d
                amradio_feat_3d = amradio_feat_3d_new
        ...
        if len(dinov2_feat_3d.shape)>2:
            dinov2_feat_3d = dinov2_feat_3d[..., 0]
        
        if len(sd_feat_3d.shape)>2:
            sd_feat_3d = sd_feat_3d[..., 0]
        
        if len(amradio_feat_3d.shape)>2:
            amradio_feat_3d = amradio_feat_3d[..., 0]
        ...
        if self.split == 'train' and flag_mask_merge:
            ...
            dinov2_feat_3d = dinov2_feat_3d[indices]
            sd_feat_3d = sd_feat_3d[indices]
            amradio_feat_3d = amradio_feat_3d[indices]
            ...
            dinov2_feat_3d = dinov2_feat_3d[vox_ind]
            
            sd_feat_3d = sd_feat_3d[vox_ind]
            
            amradio_feat_3d = amradio_feat_3d[vox_ind]
        ...
        if self.eval_all:
            return coords, feats, labels, feat_3d, dinov2_feat_3d, sd_feat_3d, amradio_feat_3d, mask, torch.from_numpy(inds_reconstruct).long()
        return coords, feats, labels, feat_3d, dinov2_feat_3d, sd_feat_3d, amradio_feat_3d, mask
...
def collation_fn(batch):
    ...
    '''
    coords, feats, labels, feat_3d, dinov2_feat_3d, sd_feat_3d, amradio_feat_3d, mask_chunk = list(zip(*batch))

    for i in range(len(coords)):
        coords[i][:, 0] *= i

    return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
        torch.cat(feat_3d), torch.cat(dinov2_feat_3d), torch.cat(sd_feat_3d), \
            torch.cat(amradio_feat_3d), torch.cat(mask_chunk)
...
def collation_fn_eval_all(batch):
    ...
    '''
    coords, feats, labels, feat_3d, dinov2_feat_3d, sd_feat_3d, amradio_feat_3d, mask, inds_recons = list(zip(*batch))
    ...
    return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
        torch.cat(feat_3d), torch.cat(dinov2_feat_3d), torch.cat(sd_feat_3d), \
            torch.cat(amradio_feat_3d), torch.cat(mask), torch.cat(inds_recons)