...
class FusedFeatureLoader(Point3DLoader):
    ...
    def __init__(self,
        ...
        self.datapath_feat_vjepa = datapath_prefix_feat_vjepa
    ...
    def __getitem__(self, index_long):
        ...
        if 'nuscenes' not in self.dataset_name:
            ...
            vjepa_processed_data = torch.load(join(
                self.datapath_feat_vjepa, scene_name+'_%d.pt'%(nn_occur)))
        else:
            # no repeated file
        ...
        if len(processed_data.keys())==2:
            ...
            vjepa_feat_3d, feat_3d, mask_chunk = vjepa_processed_data['feat'], processed_data['feat'], processed_data['mask_full']
            ...
            if self.split != 'train': # val or test set
                ...
                vjepa_feat_3d_new = torch.zeros((locs_in.shape[0], vjepa_feat_3d.shape[1]), dtype=vjepa_feat_3d.dtype)
                vjepa_feat_3d_new[mask] = vjepa_feat_3d
                vjepa_feat_3d = vjepa_feat_3d_new
        ...
        if len(vjepa_feat_3d.shape)>2:
            vjepa_feat_3d = vjepa_feat_3d[..., 0]
        ...
        if self.split == 'train' and flag_mask_merge:
            ...
            vjepa_feat_3d = vjepa_feat_3d[indices]
            ...
            vjepa_feat_3d = vjepa_feat_3d[vox_ind]
        ...
        if self.eval_all:
            return coords, feats, labels, feat_3d, vjepa_feat_3d, mask, torch.from_numpy(inds_reconstruct).long()
        return coords, feats, labels, feat_3d, vjepa_feat_3d, mask
...
def collation_fn(batch):
    ...
    '''
    coords, feats, labels, feat_3d, vjepa_feat_3d, mask_chunk = list(zip(*batch))

    for i in range(len(coords)):
        coords[i][:, 0] *= i

    return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
        torch.cat(feat_3d), torch.cat(vjepa_feat_3d), torch.cat(mask_chunk)
...
def collation_fn_eval_all(batch):
    ...
    '''
    coords, feats, labels, feat_3d, vjepa_feat_3d, mask, inds_recons = list(zip(*batch))
    ...
    return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
        torch.cat(feat_3d), torch.cat(vjepa_feat_3d), torch.cat(mask), torch.cat(inds_recons)