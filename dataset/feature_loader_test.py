...
class FusedFeatureLoader(Point3DLoader):
    ...
    def __getitem__(self, index_long):
        ...
        if self.use_shm:
            ...
            # print(self.data_paths[index], np.unique(labels_in))
        ...
        elif self.dataset_name == 'scannet_3d_200':
            scene_name = self.data_paths[index][:-15].split('/')[-1]
        else:
        ...
        if self.eval_all:
            return coords, feats, labels, feat_3d, mask, torch.from_numpy(inds_reconstruct).long(), scene_name
        return coords, feats, labels, feat_3d, mask, scene_name
...
def collation_fn_eval_all(batch):
    ...
    '''
    coords, feats, labels, feat_3d, mask, inds_recons, scene_names = list(zip(*batch))
