...
def save_fused_feature(feat_bank, point_ids, n_points, out_dir, scene_id, args):
    ...
    for n in range(args.num_rand_file_per_scene):
        ...
        lseg_path = "/nfs/datasets/jinlong_li_datasets/data_from_yoda_2d_embedding/re_my_extraction_scannet_lseg/"
        assert "lseg" in lseg_path
        
        mask_entire = torch.load(os.path.join(lseg_path, scene_id + "_%d.pt"%(n)))['mask_full']
